...
mu = 'mu20_b2bjvt_jetpt2030_trkjetpt5'
...
for jentry in xrange(nentries):
    ...
    for jpt,jeta,tjpt,jvt,thejvt,therpt,ishs,ispu in zip(tree.jpt,tree.jeta,tree.tjpt,getattr(tree,var),
                                                         tree.jjvt,tree.jrpt,tree.jishs,tree.jispu):
        if fabs(jeta)>2.4: continue
        if jpt<ptmin: continue
        if jpt>ptmax: continue
        if thejvt<0.: continue
